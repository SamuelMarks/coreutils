...
TEST test_cp_no_deref_link2(void) {
    cleanup();
    ...
    // It should fail because 'a' and 'b/a' are the same file
...
SUITE(cp_no_deref_link2_suite) {
    RUN_TEST(test_cp_no_deref_link2);
}
DEFINE_TEST_MAIN(cp_no_deref_link2_suite)
